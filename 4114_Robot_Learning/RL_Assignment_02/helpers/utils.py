...
for action_conv in [8, 7, 6, 3, 0, 1, 2,
                    5]:  # [SE, S, SW, W, NW, N, NE, E], this is [NW, N, NE, E, SE, S, SW, W] rotated 180°
...
def getNextStatesRewardsAndProbabilities(state, action, rewards=rewards, deviation=True):
    ...
    if deviation:
        ...
        probabilities = [0.1, 0.7, 0.2]  # deviation probabilities defined on the assignment sheet
    else:
    ...
    for cell in cells:
        if cell in terminalStates:
            ...
            if cell in stars:
                ...
                ax.add_patch(rect)
            if pi[cell][0] == 1.:
                ax.arrow(cell[1] + 0.85, cell[0] + 0.85, -0.7, -0.7, head_width=0.2, head_length=0.2,
                         length_includes_head=True,
                         fill=True)
            elif pi[cell][1] == 1.:
                ax.arrow(cell[1] + 0.5, cell[0] + 0.9, 0, -0.8, head_width=0.2, head_length=0.2,
                         length_includes_head=True,
                         fill=True)
            elif pi[cell][2] == 1.:
                ax.arrow(cell[1] + 0.15, cell[0] + 0.85, 0.7, -0.7, head_width=0.2, head_length=0.2,
                         length_includes_head=True,
                         fill=True)
            elif pi[cell][3] == 1.:
                ax.arrow(cell[1] + 0.1, cell[0] + 0.5, 0.8, 0, head_width=0.2, head_length=0.2,
                         length_includes_head=True,
                         fill=True)
            elif pi[cell][4] == 1.:
                ax.arrow(cell[1] + 0.15, cell[0] + 0.15, 0.7, 0.7, head_width=0.2, head_length=0.2,
                         length_includes_head=True,
                         fill=True)
            elif pi[cell][5] == 1.:
                ax.arrow(cell[1] + 0.5, cell[0] + 0.1, 0, 0.8, head_width=0.2, head_length=0.2,
                         length_includes_head=True,
                         fill=True)
            elif pi[cell][6] == 1.:
                ax.arrow(cell[1] + 0.85, cell[0] + 0.15, -0.7, 0.7, head_width=0.2, head_length=0.2,
                         length_includes_head=True,
                         fill=True)
            else:
                ax.arrow(cell[1] + 0.9, cell[0] + 0.5, -0.8, 0, head_width=0.2, head_length=0.2,
                         length_includes_head=True,
                         fill=True)
    ax.tick_params(
        axis='x',
...
def drawGrid(cells=states, terminalStates=terminalStates, obstacles=obstacles, stars=stars):
    ...
    plt.legend(handlelength=2, handleheight=2.7, handles=[red_patch, blue_patch, green_patch], loc='center left',
