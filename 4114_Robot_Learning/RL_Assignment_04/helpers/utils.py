...
permittedAcceleration = np.dstack(
    [velocityFeatures[:, :, 0] < 7, velocityFeatures[:, :, 0] > 0, velocityFeatures[:, :, 1] > -1])
...
def drawTrajectory(moves, actions, velocities, cells=states, circuit=circuit, finishLine=finishLine,
    ...
    for i in range(len(actions)):
        ...
        ax.arrow(x + 0.5, y + 0.5, 0.95 * v0, -0.95 * v1, head_width=0.2, head_length=0.2, length_includes_head=True,
                 fill=True, color='r')
